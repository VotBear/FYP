...
public class VariableManager : MonoBehaviour {
	...
	void UpdateValue(int row){  
		...
		if (row == 1){ 
			...
			if (currentVar.val_1 == C.TYPE_EVENT){ 		// Event 
				...
				if (val == 0 || EventListManager.Instance.GetEventAmount(val-1) <= 1){
					for(int i=row; i<4; ++i) rows[i].SetActive(false);   
				} else {
					UpdateRow(3);
				}
				...
				if (currentVar.val_2 == C.VAL_SELECT){
					for(int i=row; i<4; ++i) rows[i].SetActive(false);  
				} else {
					UpdateRow(3);
				}
			...
			if (currentVar.val_1 == C.TYPE_EVENT){ 		// Event 
				...
				if (currentVar.val_3 == C.SEL_ANY){ 
					toolTip.text = string.Format(C.SELECTION_TOOLTIP[C.SEL_ANY], EventListManager.Instance.GetEventName(eventId));
					for(int i=row; i<4; ++i) rows[i].SetActive(false);   

				} else if (currentVar.val_3 == C.SEL_NONE){ 
					toolTip.text = string.Format(C.SELECTION_TOOLTIP[C.SEL_NONE], EventListManager.Instance.GetEventName(eventId));
					for(int i=row; i<4; ++i) rows[i].SetActive(false);   
			...
			if (currentVar.val_1 == C.TYPE_EVENT){ 		// Event 
				...
				if (currentVar.val_3 == C.SEL_ALL) {
					if (currentVar.val_4 == C.AGG_ALL) 
						stringFormat = C.AGGREGATE_TOOLTIP[C.AGG_ALL];

					else if (currentVar.val_4 == C.AGG_SUM) 
						stringFormat = C.AGGREGATE_TOOLTIP[C.AGG_SUM];

					else if (currentVar.val_4 == C.AGG_MIN) 
						stringFormat = C.AGGREGATE_TOOLTIP[C.AGG_MIN];

					else if (currentVar.val_4 == C.AGG_MAX) 
						stringFormat = C.AGGREGATE_TOOLTIP[C.AGG_MAX];

					toolTip.text = string.Format(stringFormat, EventListManager.Instance.GetEventName(eventId));
				}  
				else {
					int curVal = val;
					if (currentVar.val_3 == C.SEL_SPECIFIC){
						curVal = val+1;
						stringFormat = C.SELECTION_TOOLTIP[C.SEL_SPECIFIC];
					}
					else if (currentVar.val_3 == C.SEL_ATLEAST){
						stringFormat = C.SELECTION_TOOLTIP[C.SEL_ATLEAST];
					}
					else if (currentVar.val_3 == C.SEL_ATMOST){
						stringFormat = C.SELECTION_TOOLTIP[C.SEL_ATMOST];
					}
					else if (currentVar.val_3 == C.SEL_EXACTLY){
						stringFormat = C.SELECTION_TOOLTIP[C.SEL_EXACTLY];
					}
					toolTip.text = string.Format(stringFormat, curVal, EventListManager.Instance.GetEventName(eventId));
				}
			} 
		} 
	...
	public void UpdateRow(int row){ 
		...
		if (row == 1){
			...
			if (currentVar.onlyEvent) options.RemoveAt(2); 						   //remove variable option if first variable
