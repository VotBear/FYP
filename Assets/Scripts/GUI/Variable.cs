...
public class Variable : MonoBehaviour {
	...
	public void Reset(){
		// Reset all event rules into undefined
		if (val_1 == C.TYPE_EVENT){
			ClearValues();
			val_1 = C.TYPE_EVENT;
			UpdateText();
		}
	...
	private string CurrentEventName(){
		return "[" + EventListManager.Instance.GetEventName(val_2 - 1) + "]";
	}
	...
	public string GetName(){
		if (val_1 == C.TYPE_UNDEF){				
			...
			} else if (EventListManager.Instance.GetEventAmount(val_2 - 1) <= 1){  
				// singular event
